...
const TrustedBy = () => {
  return (
    ...
    className="flex flex-col items-center px-4 sm:px-12 lg:px-24 xl:px-40 gap-4 text-gray-700 dark:text-white/80">
      <motion.h3 
      ...
      className="flex items-center justify-center flex-wrap gap-10 m-4">
        {company_logos.map((logo, index) => (
          ...
          key={index}
            src={logo}
            alt="company logo"
            className="max-h-12 sm:max-h-12 dark:drop-shadow-xl"
          />
        ))}
      </motion.div>
    </motion.div>
